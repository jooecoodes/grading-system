...
if (isset($_GET['user'])) {
    ...
    if (isset($_SESSION['teacherId'])) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $token = $studToken;
            $studQrName = $studFname . "_" . $studLname;

            $qrSize = 300;
            // Generate the QR code image and save it to a file
            QRcode::png($token, "../../assets/qr/$studFullName.png", QR_ECLEVEL_H, $qrSize);
...
?>
            ...
            <body>
                <form enctype="multipart/form-data" action="qr_handler.php" method="POST">
                    ...
                    <label for="pfp">Profile:</labame="pfp"><br>
                ...
                <form action="download_qr.php" method="POST">
                    <input type="hidden" name="download-qr" value="1">
                    <input type="hidden" name="student-qr-name" value="<?= $studQrName ?>">
                    <label for="downloadQrField">Download QR Code:</label>
                    <input type="submit" value="Download" id="downloadQrField">
                </form>
